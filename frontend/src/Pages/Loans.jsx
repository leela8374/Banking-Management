...
const Loans = () => {
  ...
  const makePayment = async (id) => {
    const paymentData = { id, amount: Number(paymentLoanAmount) };
    try {
      const response = await axios.post('http://localhost:6001/repay-loan', paymentData);
      alert("Loan payment successful");
      setPaymentLoanAmount(0);
      setPaymentLoanId(null); // Reset paymentLoanId after successful payment
      fetchLoansData();
    } catch (err) {
      console.error('Error making payment:', err); // Added error logging
      ...
      <div className="user-loans-page">
              ...
              {loans && loans.filter(loan => loan.customerId === userid).map((loan) => {
                return (
                  <div className="user-loan" key={loan._id}>
                    ...
                    {loan.loanStatus === 'approved' && loan.balance>0 &&
                      <div>
        ...
        <div className="user-loans-container2">
          <div className="new-user-loans-container">
            ...
            <select className="form-select" aria-label="Default select example" value={newLoanType} onChange={(e) => setNewLoanType(e.target.value)}>
            ...
            <p> * I hereby agree to all the terms & conditions to make this loan. I agree to pay interest with the dynamic interests depending upon the market conditions. I agree to repay the loan before the deadline.</p>
